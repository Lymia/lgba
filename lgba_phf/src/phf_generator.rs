...
use crate::phf_shared::HashKey;
use alloc::{vec, vec::Vec};
use core::hash::Hash;
use rand::{distributions::Standard, rngs::SmallRng, Rng, SeedableRng};

// Modified from 5 to 4.
// A lambda of 4 ensures a power of 2 size for the displacements table.
...
fn try_generate_hash<H: Hash>(entries: &[H], key: HashKey) -> Option<HashState> {
    struct Bucket {
        idx: usize,
        keys: Vec<usize>,
    }
    ...
    let hashes: Vec<_> = entries
        .iter()
        .map(|entry| crate::phf_shared::hash(entry, &key))
        .collect();
        ...
        for d1 in 0..(table_len as u32) {
            'disps: for d2 in 0..(table_len as u32) {
                ...
                for &key in &bucket.keys {
                    let idx = (crate::phf_shared::displace(hashes[key].f1, hashes[key].f2, d1, d2)
