...
fn download_fonts() -> Result<Vec<CharacterInfo>> {
    ...
    for (char, glyph) in misaki_font.glyphs() {
        ...
        let mut y_off = if glyph.height() != 8 {
...
fn make_glyphs_file(glyphs: GlyphData) {
    ...
    for (i, byte) in glyphs.data.iter().enumerate() {
        if i % 32 == 0 {
            data_str.push_str("    ");
        }
        data_str.push_str(&format!("0x{byte:02x},"));
        if i % 32 == 31 {
            data_str.push_str("\n");
        }
    }
    ...
    let mut low_plane_str = String::new();
    for word in low_plane {
        low_plane_str.push_str(&format!("0x{word:04x}, "))
    }
    low_plane_str.pop();
    low_plane_str.pop();
    ...
    for glyph in glyphs.glyph_map.keys() {
        entries.push(*glyph);
    }
    assert!(entries.len() < 1020);
    while entries.len() < 1024 {
        entries.push(0xE000 + entries.len() as u16);
    }
    let phf = lgba_phf::phf_generator::generate_hash(&entries);
    println!("{:?}", phf.disps);
    assert!(phf.disps.len().is_power_of_two());
    ...
    let raw_source = format!(
        ...
        \n\
        pub static LOW_PLANE_BITMAP: [u16; 16] = [{low_plane_str}];\n\
        \n\
        #[repr(align(4))]\n\
        struct Align([u8; 1024 * 8]);\n\
        static RAW_FONT_DATA: Align([u8; 1024 * 8]) = Align([\n\
            {data_str}\n\
        ]);\n\
