...
extern "C" {
    fn __lgba_internal_WramTransferBuf(src: *const u8, dst: *mut u8, count: usize);
    fn __lgba_internal_WramReadByte(src: *const u8) -> u8;
    fn __lgba_internal_WramVerifyBuf(buf1: *const u8, buf2: *const u8, count: usize) -> bool;
}
...
pub unsafe fn read_raw_buf(dst: &mut [u8], src: usize) {
    if !dst.is_empty() {
        __lgba_internal_WramTransferBuf(src as _, dst.as_mut_ptr(), dst.len());
    }
}
...
pub unsafe fn write_raw_buf(dst: usize, src: &[u8]) {
    if !src.is_empty() {
        __lgba_internal_WramTransferBuf(src.as_ptr(), dst as _, src.len());
    }
}
...
pub unsafe fn verify_raw_buf(buf1: &[u8], buf2: usize) -> bool {
    if !buf1.is_empty() {
        __lgba_internal_WramVerifyBuf(buf1.as_ptr(), buf2 as _, buf1.len() - 1)
    } else {
        true
    }
}
...
pub unsafe fn read_raw_byte(src: usize) -> u8 {
    __lgba_internal_WramReadByte(src as _)
}
