...
//! ## Save media types
//!
//! There are, broadly speaking, three different kinds of save media that can be found in official
//! Game Paks:
//!
//! * Battery-Backed SRAM: The simplest kind of save media, which can be accessed like normal
//!   memory. You can have SRAM up to 32KiB, and while there exist a few variants this does not
//!   matter much for a game developer.
//! * EEPROM: A kind of save media based on very cheap chips and slow chips. These are accessed
//!   using a serial interface based on reading/writing bit streams into IO registers. This memory
//!   comes in 8KiB and 512 byte versions, which unfortunately cannot be distinguished at runtime.
//! * Flash: A kind of save media based on flash memory. Flash memory can be read like ordinary
//!   memory, but writing requires sending commands using multiple IO register spread across the
//!   address space. This memory comes in 64KiB and 128KiB variants, which can thankfully be
//!   distinguished using a chip ID.
//!
//! As these various types of save media cannot be easily distinguished at runtime, the kind of
//! media in use should be set manually.
//!
//! ## Setting save media type
//!
//! To use save media in your game, you must set which type to use. This is done by calling one of
//! the following functions at startup:
//!
...
//! ## Using save media
//!
//! To access save media, create a [`SaveAccess`] object using its [`open`] or
//! [`open_with_timeout`] methods. It contains the methods used to read or write save media.
//!
//! Reading data from the save media is simple. Use [`read`] to copy data from an offset in the
//! save media into a buffer in memory.
//!
//! Writing to save media requires you to prepare the area for writing by calling the
//! [`prepare_write`] method to return a [`SavePreparedBlock`], which contains the actual
//! [`write`] method.
//!
//! The `prepare_write` method leaves everything in a sector that overlaps the range passed to it
//! in an implementation defined state. On some devices it may do nothing, and on others, it may
//! clear the entire range to `0xFF`.
//!
//! Because writes can only be prepared on a per-sector basis, a clear on a range of `4000..5000`
//! on a device with 4096 byte sectors will actually clear a range of `0..8192`. Use
//! [`sector_size`] to find the sector size, or [`align_range`] to directly calculate the range of
//! memory that will be affected by the clear.
//!
//! [`open`]: SaveAccess::open
//! [`open_with_timeout`]: SaveAccess::open_with_timeout
//! [`read`]: SaveAccess::read
...
//!
//! * SRAM is simply a form of battery backed memory, and has no particular performance
//!   characteristics. Reads and writes at any alignment are efficient. Furthermore, no timer is
//!   needed for accesses to this type of media. `prepare_write` does not immediately erase any
//!   data.
//! * Non-Atmel flash chips have a sector size of 4096 bytes. Reads and writes to any alignment are
//!   efficient, however, `prepare_write` will erase all data in an entire sector before writing.
//! * Atmel flash chips have a sector size of 128 bytes. Reads to any alignment are efficient,
...
pub struct MediaInfo {
    ...
    /// The power-of-two size of each sector. Zero represents a sector size of 0, implying sectors
    /// are not in use.
    ///
...
impl MediaInfo {
    /// Returns the sector size of the save media. It is generally optimal to write data in blocks
    /// that are aligned to the sector size.
    #[must_use]
...
fn set_save_implementation(access_impl: &'static dyn RawSaveAccess) {
    ...
    assert!(access.is_none(), "Cannot initialize the save media engine more than once.");
...
impl SaveAccess {
    ...
    ///
    /// You must have initialized the save manager beforehand to use a specific type of media
    /// before calling this method.
    pub fn open() -> Result<SaveAccess, Error> {
        Self::new(None)
    }
    ...
    /// You must have initialized the save manager beforehand to use a specific type of media
    /// before calling this method.
    pub fn open_with_timeout(timer: TimerId) -> Result<SaveAccess, Error> {
        Self::new(Some(timer))
    }
    ...
    pub fn prepare_write(&mut self, range: Range<usize>) -> Result<SavePreparedBlock, Error> {
        ...
        Ok(SavePreparedBlock { parent: self, range })
    }
}

/// A block of save memory that has been prepared for writing.
pub struct SavePreparedBlock<'a> {
    parent: &'a mut SaveAccess,
    range: Range<usize>,
}
impl<'a> SavePreparedBlock<'a> {
    /// Writes a given buffer into the save media.
    ///
    /// Multiple overlapping writes to the same memory range without a separate call to
    /// `prepare_write` will leave the save data in an unpredictable state. If an error is
    /// returned, the contents of the save media is unpredictable.
    ...
    ///
    /// This function will verify that the write has completed successfully, and return an error if
    /// it has not done so.
    ///
    /// Multiple overlapping writes to the same memory range without a separate call to
    /// `prepare_write` will leave the save data in an unpredictable state. If an error is
...
///
/// Flash save media is generally very slow to write to and relatively fast to read from. It is
/// the only real option if you need larger save data.
///
/// This creates a marker in the ROM that allows emulators to understand what save type the Game
/// Pak uses, and configures the save manager to use the given save type.
///
/// Only one `init_*` function may be called in the lifetime of the program.
pub fn init_flash_64k() {
