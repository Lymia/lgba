...
const FLASH_PORT_BANK: Register<u8> = unsafe { Register::new(0xE000000) };
const FLASH_PORT_A: Register<u8> = unsafe { Register::new(0xE005555) };
const FLASH_PORT_B: Register<u8> = unsafe { Register::new(0xE002AAA) };
const FLASH_DATA: RegArray<u8, 65536> = unsafe { RegArray::new(0xE000000) };
...
impl ChipInfo {
    ...
    fn erase_sector(&self, sector: usize, timeout: &mut Timeout) -> Result<(), Error> {
        ...
        start_flash_command();
        FLASH_DATA
            .index(offset & BANK_MASK)
            .write(CMD_ERASE_SECTOR_CONFIRM);
